...
@implementation JHCQRCodeScanView
...
- (void)initialization
{
    _scanAnimationStyle = JHCScanAnimatinStyle_Default;
    _cornerLocation = JHCScanCornerLoction_Default;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.cornerLocation == JHCScanCornerLoction_Inside)
    {
    ...
    else if (self.cornerLocation == JHCScanCornerLoction_Outside)
    {
...
- (void)addTimer
{
    ...
    if (_scanAnimationStyle == JHCScanAnimatinStyle_Grid) {
...
- (void)beginRefreshUI
{
    ...
    if (self.scanAnimationStyle == JHCScanAnimatinStyle_Grid)
    {
        if (flag) {
...
- (void)setScanAnimationStyle:(JHCScanAnimatinStyle)scanAnimationStyle
{
...
- (void)setCornerLocation:(JHCScanCornerLoction)cornerLocation
{
...
{
    if (_scanningline) return _scanningline;
    ...
    if (!url) {
        if (self.scanImageName) {
            UIImage *image = [UIImage imageNamed:_scanImageName];
            if (image) {
    ...
    if (!image) {
        image = [UIImage imageNamed:_scanImageName];
    }
    _scanningline.image = image;
    return _scanningline;
}
