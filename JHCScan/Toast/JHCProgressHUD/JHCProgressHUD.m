...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"JHCProgressHUD needs to be accessed on the main thread.");

CGFloat const JHCProgressMaxOffset = 1000000.f;
...
@interface JHCProgressHUDRoundedButton : UIButton
@end


@implementation JHCProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    JHCProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    JHCProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (JHCProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            JHCProgressHUD *hud = (JHCProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = JHCProgressHUDAnimationFade;
    _mode = JHCProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(JHCProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == JHCProgressHUDAnimationZoom) {
        type = animatingIn ? JHCProgressHUDAnimationZoomIn : JHCProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == JHCProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == JHCProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == JHCProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == JHCProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }

        bezelView.alpha = animatingIn ? 1.0f : 0.f;
...
- (void)done {
    ...
    JHCProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<JHCProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    UIButton *button = [JHCProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    else if (mode == JHCProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JHCProgressHUDModeDeterminate || mode == JHCProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
    ...
    else if (mode == JHCProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        UIActivityIndicatorView *appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[JHCProgressHUD class]]];
...
- (void)updateBezelMotionEffects {
    ...
    if (self.defaultMotionEffectsEnabled) {
        ...
        // 解决 Main Thread Checker: UI API called on a background thread:
...
- (void)setMode:(JHCProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == JHCProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = JHCProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(JHCProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    JHCProgressHUDBackgroundStyle style = self.style;
    if (style == JHCProgressHUDBackgroundStyleBlur) {
...
@implementation JHCProgressHUDRoundedButton
